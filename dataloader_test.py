...
def main(args):
    ...
    if config.pretrain:
        if os.path.isfile(config.pretrain):
            ...
            # # 找出匹配成功的键
            # matched_keys = [key for key in checkpoint['model_state_dict'].keys() if key in model_state_dict]

            # # 找出未匹配的键（在state_dict中但在model中不存在）
            # unmatched_keys = [key for key in checkpoint['model_state_dict'].keys() if key not in model_state_dict]

            # # 打印匹配和未匹配的键
            # print("K400_pretrain_Matched Keys:")   # 512
            # for key in matched_keys:
            #     print(key)

            # print("\nK400_pretrain_Unmatched Keys (ignored by strict=False):")   # 960
    ...
    classes = text_prompt(train_data)    # torch.Size([51, 77])
    ...
    for i,(images, list_id) in enumerate(train_loader):
        ...
        with autocast():
            if config.solver.loss_type in ['NCE', 'DS']:
                ...
                print("model before image shape:", images.shape)
                print("model before texts shape:",texts.shape)
