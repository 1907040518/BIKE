...
# 将图像数据缩放到特定的大小。它接受一个图像数组 img 和一个目标大小 size 
...
class CoviarDataSet(data.Dataset):
    ...
    def _get_train_frame_index(self, num_frames, seg):
        ...
        seg_begin, seg_end = get_seg_range(num_frames, self._num_segments, seg,
                                                 representation=self._representation)   # get_seg_range 计算当前段的帧范围。

        # Sample one frame from the segment.
        v_frame_idx = random.randint(seg_begin, seg_end - 1)
        return get_gop_pos(v_frame_idx, self._representation)  # get_gop_pos 计算选定帧的 GOP 位置 第几个GOP 和 GOP中的位置
    ...
    def __getitem__(self, index):
        ...
        if self._is_train:
            video_path, label, num_frames = random.choice(self._video_list)   # 从 self._video_list 中随机选择一个视频路径、标签和帧数。
        else:
            video_path, label, num_frames = self._video_list[index]

        frames = []
        for seg in range(self._num_segments):   # 获取num segment
