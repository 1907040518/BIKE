...
def main():
    ...
    model = torch.nn.DataParallel(model, device_ids=args.gpus).cuda(2)
...
def train(train_loader, model, criterion, optimizer, epoch, cur_lr):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # print(f"input_var size: {input_var.size()}, target_var size: {target_var.size()}")

        output = model(input_var)
        # print(f"output size: {output.size()}, target_var size: {target_var.size()}")
        # print(output.shape)
        # 模型输出大小的调整，保证与标签的大小对应，计算损失
