...
class GroupRandomHorizontalFlip(object):
    ...
    def __call__(self, img_group, is_mv=False):
        if random.random() < 0.5:
            ...
            first_img_channels = ret[0].shape[-1]
            if first_img_channels == 2:
...
def resize_mv(img, shape, interpolation):
    print(f"Data type of input image in resize_mv: {img.dtype}")
    print(f"Input image to resize_mv shape: {img.shape}")
    resized_channels = []
    for i in range(2):
        channel = img[..., i]
        print(f"Shape of channel {i} before resize: {channel.shape}")
        resized_channel = cv2.resize(channel, shape, interpolation)
        print(f"Shape of channel {i} after resize: {resized_channel.shape}")
        resized_channels.append(resized_channel)
    result = np.stack(resized_channels, axis=2)
    print(f"Output image from resize_mv shape: {result.shape}")
    return result
...
class GroupMultiScaleCrop(object):
    ...
    def __call__(self, img_group):
        ...
        print(f"Before cropping, image group shape: {[img.shape for img in img_group]}")
        crop_w, crop_h, offset_w, offset_h = self._sample_crop_size(im_size)
        print(f"Calculated crop width: {crop_w}, crop height: {crop_h}")
        print(f"Calculated horizontal offset: {offset_w}, vertical offset: {offset_h}")
        crop_img_group = [img[offset_w:offset_w + crop_w, offset_h:offset_h + crop_h] for img in img_group]
        print(f"After cropping, image group shape: {[img.shape for img in crop_img_group]}")
        if crop_img_group[0].shape[2] == 3:
            ...
            ret_img_group = [resize_mv(img, (self.input_size[0], self.input_size[1]), cv2.INTER_LINEAR)
                             for img in crop_img_group]
        print(f"After resizing, image group shape: {[img.shape for img in ret_img_group]}")
        return ret_img_group

    def _sample_crop_size(self, im_size):
        image_w, image_h = im_size[0], im_size[1]
        print(f"Original image width: {image_w}, height: {image_h}")
        base_size = min(image_w, image_h)
        crop_sizes = [int(base_size * x) for x in self.scales]
        print(f"Generated crop sizes: {crop_sizes}")
        crop_h = [self.input_size[1] if abs(x - self.input_size[1]) < 3 else x for x in crop_sizes]
        crop_w = [self.input_size[0] if abs(x - self.input_size[0]) < 3 else x for x in crop_sizes]
        print(f"Adjusted crop widths: {crop_w}, crop heights: {crop_h}")
        pairs = []
        for i, h in enumerate(crop_h):
            for j, w in enumerate(crop_w):
                if abs(i - j) <= self.max_distort:
                    pairs.append((w, h))
        print(f"Valid crop size pairs: {pairs}")
        crop_pair = random.choice(pairs)
        print(f"Selected crop pair: {crop_pair}")
        if not self.fix_crop:
            w_offset = random.randint(0, image_w - crop_pair[0])
            h_offset = random.randint(0, image_h - crop_pair[1])
        else:
            w_offset, h_offset = self._sample_fix_offset(image_w, image_h, crop_pair[0], crop_pair[1])
        print(f"Calculated offset: horizontal {w_offset}, vertical {h_offset}")
        return crop_pair[0], crop_pair[1], w_offset, h_offset

    def _sample_fix_offset(self, image_w, image_h, crop_w, crop_h):
        offsets = self.fill_fix_offset(self.more_fix_crop, image_w, image_h, crop_w, crop_h)
        print(f"Generated fixed offsets: {offsets}")
    ...
    def fill_fix_offset(more_fix_crop, image_w, image_h, crop_w, crop_h):
        ...
        print(f"Fixed offset candidates: {ret}")
        return ret

def get_compress_augmentation(training, config):

    if training:
        if config.data.modality in ['mv', 'residual']:
            scales = [1, .875, .75]
        else:
            scales = [1, .875, .75, .66]
        ...
        return torchvision.transforms.Compose(
            [GroupMultiScaleCrop(config.data.input_size, scales),
