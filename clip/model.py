...
class InstanceAwareDynamicFusion(nn.Module):
    ...
    def forward(self, iframe_feats, mv_feats, residual_feats, attribute_tokens, base_weights):
        ...
        # 适度保护预训练空间：文本可以 detach，因为文本分支通常是提供条件的
        attr_detached = attribute_tokens.detach() 
        K = self.k_proj(attr_detached)  
        V = self.v_proj(attr_detached)  
        
        pooled_features = []
        enhanced_modalities = [] # 存储注入了文本语义的增强视觉特征
        
        for i, mod_feat in enumerate(modalities):
            # 🔥 涨点修改3：移除 mod_feat 的 detach()！
            # 如果你怕破坏主干，应该在优化器里给融合模块设置单独的 learning rate (比如大10倍)，
            # 而不是在这里阻断梯度。融合网络需要告诉主干："提取什么样的特征有助于融合"。
            
            Q = self.q_proj[i](mod_feat) # (B, T, hidden_dim)
            # 计算注意力 (B, T, N)
            attn_scores = torch.bmm(Q, K.transpose(1, 2)) / (K.size(-1)**0.5) 
            attn_weights = F.softmax(attn_scores, dim=-1)
            
            # (B, T, hidden_dim) 每一帧都获得了对应的属性关注！
            semantic_injected_feat = torch.bmm(attn_weights, V) 
            
            # 🔥 涨点修改4：将文本语义特征映射回视觉维度，做残差相加！
            # 这使得即使 router 权重一样，视觉特征本身也拥有了 Instance-Aware 的属性！
            feat_delta = self.feat_inject_proj[i](semantic_injected_feat)
            enhanced_mod = mod_feat + self.inject_scale[i] * feat_delta
            enhanced_modalities.append(enhanced_mod)
            
            # 保留时序维度用于路由预测，不使用 mean(dim=1)
            pooled_features.append(semantic_injected_feat)

        # --- 生成 帧级别(Frame-Aware) 的残差权重 ---
        global_attr = attr_detached.mean(dim=1).unsqueeze(1).expand(-1, T, -1) # (B, T, semantic_dim)
        
        # 拼接特征: (B, T, 3 * hidden_dim + semantic_dim)
        router_input = torch.cat(pooled_features + [global_attr], dim=-1) 
        
        # weight_residual: (B, T, 3) 🔥 每一帧都有独立的权重调整！
        weight_residual = self.dynamic_router(router_input) 
        
        # 使用可学习的 gate 控制，替代 clamp 和 zero-init
        weight_residual = weight_residual * self.router_gate 
        
        # --- 基础权重 + 动态残差调整 ---
        # base_weights 扩展为 (B, T, 3)
        base_weights_batch = base_weights.unsqueeze(0).unsqueeze(0).expand(B, T, -1)
        final_weights = F.softmax(base_weights_batch + weight_residual, dim=-1)
        
        # --- 最终加权融合 ---
        # 🔥 使用语义增强后的特征进行融合
        stacked_mods = torch.stack(enhanced_modalities, dim=2)  # (B, T, 3, D)
        weight_view = final_weights.unsqueeze(-1)  # (B, T, 3, 1)
