...
# 无论从哪里调用本脚本，都能正确找到 clip / modules / utils / Coviar 等包
...
MODAL_FILLS  = ["#2E86AB33", "#E8485533", "#3BB27333"]  # 半透明填充
...
# ============================================================
# 📦 数据收集器：跨 batch 积累多个视频的可视化数据
# ============================================================
class VisualizationBuffer:
    """
    在 validate 循环中积累多个视频的权重数据，
    最后统一调用 render() 生成顶会级对比图。
    """
    def __init__(self, max_videos: int = 6):
    ...
    def add(self, video_path: str, final_weights: np.ndarray,
        ...
        """
        Args:
            video_path:        视频路径（取文件名作标题）
            final_weights:     shape (T, 3)，三模态融合权重时序
            attn_weights_list: list of (T, N, N) 或 (T, N)，每个模态的注意力
            gt_label:          真实类别名
            pred_label:        预测类别名
            is_correct:        是否预测正确
        """
        if len(self.records) >= self.max_videos:
            return
        self.records.append({
            "name":        Path(video_path).stem[:28],   # 截断避免过长
            "weights":     final_weights,                # (T, 3)
            "attns":       attn_weights_list,
            "gt":          gt_label,
            "pred":        pred_label,
            "correct":     is_correct,
        })
    ...
    def ready(self) -> bool:
        return len(self.records) >= self.max_videos

    def render(self, save_dir: str, tag: str = "comparison"):
        if not self.records:
            return
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f"modal_weight_{tag}.pdf")
        _render_comparison_figure(self.records, save_path)
        print(f"[Visualizer] 顶会图已保存 → {save_path}")
        self.records.clear()   # 清空，准备下一批


# ============================================================
# 🖼️  核心绘图函数
# ============================================================
def _render_comparison_figure(records: list[dict], save_path: str):
    """
    生成三区域复合图：
      (A) 多视频权重时序曲线对比（主图）
      (B) 模态贡献均值热图
    ...
    T        = records[0]["weights"].shape[0]
    t_axis   = np.arange(T)

    # ── 画布布局 ────────────────────────────────────────────
    fig = plt.figure(figsize=(14, 3.2 * n_videos + 3.5))
    outer = gridspec.GridSpec(
        2, 1,
        height_ratios=[n_videos * 3.2, 3.5],
        hspace=0.38,
    )
    ...
    for vi, rec in enumerate(records):
        ...
        for mi, (mname, mcolor, mfill) in enumerate(
            zip(MODAL_KEYS, MODAL_COLORS.values(), MODAL_FILLS)
        ):
            ax.plot(
                t_axis, w[:, mi],
                color=mcolor, linewidth=1.8,
                label=mname if vi == 0 else "_nolegend_",
                zorder=3,
            )
            ax.fill_between(
                t_axis, 0, w[:, mi],
                color=mfill, zorder=2,
            )
        ...
        ax.set_title(
            ...
            color=color_tag, loc="left", pad=3,
        )
        ax.set_ylim(0, 1.05)
        ax.set_xlim(0, T - 1)
        ...
        ax.tick_params(axis="x", labelbottom=(vi == n_videos - 1))
    ...
    # 统一图例（放在第一个子图右上角）
    handles = [
        plt.Line2D([0], [0], color=c, linewidth=2, label=m)
    ...
    fig.text(
        0.5, 0.98,
        "Dynamic Modal Weight Balancing Across Diverse Video Scenes",
        ha="center", va="top",
    ...
    mean_weights = np.stack(
        [rec["weights"].mean(axis=0) for rec in records], axis=0
    )  # (n_videos, 3)
    ...
    ax_B.set_xticklabels(MODAL_KEYS, fontsize=8)
    ax_B.set_yticks(range(n_videos))
    ax_B.set_yticklabels(
        [r["name"][:18] for r in records], fontsize=7
    )
    ax_B.set_title("Mean Modal Contribution", fontsize=9, fontweight="bold")
    ...
    plt.colorbar(im_B, ax=ax_B, shrink=0.8, pad=0.03)

    # ── 区域 C：注意力矩阵（三模态并排） ───────────────────
    gs_C = gridspec.GridSpecFromSubplotSpec(
        1, 3,
        subplot_spec=gs_BC[1],
        wspace=0.15,
    )

    for mi, mname in enumerate(MODAL_KEYS):
        ax_C = fig.add_subplot(gs_C[mi])

        # 聚合所有视频该模态的注意力（取均值）
        attn_list = []
        for rec in records:
            if mi < len(rec["attns"]) and rec["attns"][mi] is not None:
                a = rec["attns"][mi]
                # 兼容 (T, N, N) 或 (T, N) 或 (N, N)
                if a.ndim == 3:
                    a = a.mean(axis=0)   # → (N, N)
                elif a.ndim == 1:
                    a = np.outer(a, a)   # → (N, N)
        ...
        if attn_list:
            ...
            plt.colorbar(im_C, ax=ax_C, shrink=0.75, pad=0.03)
        else:
        ...
        ax_C.set_title(
            f"{mname}\nAttention",
            fontsize=8.5, fontweight="bold",
            color=list(MODAL_COLORS.values())[mi],
        )
        ax_C.set_xlabel("Token Index", fontsize=7.5)
        if mi == 0:
            ax_C.set_ylabel("Token Index", fontsize=7.5)
        else:
...
def _annotate_dominant_regions(ax, t_axis, dominant, T):
    """
    用淡色背景块标注每段时间内主导模态，增强可读性。
    """
    colors_bg = ["#2E86AB18", "#E8485518", "#3BB27318"]
    i = 0
    while i < T:
        j = i
...
def save_visualizations(final_weights, attn_weights_list, video_path, save_dir):
    ...
    # 1. 绘制 Temporal Weight Heatmap (模态权重随时间变化)
    plt.figure(figsize=(10, 4))
    
    # final_weights 的形状是 (T, 3)，转置为 (3, T) 方便 X轴表示时间，Y轴表示模态
    weights_T = final_weights.T 
    
    # 注意：这里的标签顺序 [I-frame, MV, Residual] 必须和你 forward 里 modalities 的顺序一致！
...
def get_parser():
    ...
    parser.add_argument("--visualize_video", type=str, default=None, 
                        help="指定要可视化的视频名片段，例如 'climb_stairs/Piano'")
...
def validate(val_loader, classes, device, model, video_head, config, n_class, logger,
             ...
             test_dataset=None, args=None):          # 🔥 新增：传入数据集用于反查路径
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    prediction_records = []
    # ── 在 validate 函数开头，with torch.no_grad(): 之前 ──────
    viz_buffer = VisualizationBuffer(max_videos=6)   # 🔥 收集6个视频后统一出图
    with torch.no_grad():
        if classes is None:
        ...
        for i, batch in enumerate(val_loader):
            if config.data.modality in ["mv", "residual", "iframe"]:
                ...
                if attribute_prompt_enabled and getattr(clip_model, "attribute_guided_fusion", None) is not None:
                    ...
                    for cls_idx in range(n_class):
                        ...
                        fused_feats, final_weights, _, attn_weights_list = clip_model.attribute_guided_fusion(
            ...
            if save_predictions_path is not None:
                ...
                for sample_idx in range(b):
                    ...
                    # 🔥 升级版：多视频可视化拦截 + 容错逻辑
                    # ============================================================
                    if args is not None and args.visualize_video is not None:
                        target_keywords = [k.strip() for k in args.visualize_video.split(',')]

                        if any(kw in video_path for kw in target_keywords):
                            if 'final_weights' in locals() and final_weights is not None:
                                w_np   = final_weights[sample_idx].cpu().numpy()       # (T, 3)
                                a_np   = [a[sample_idx].cpu().numpy()
                                        for a in attn_weights_list]                  # list of (T,N,N)

                                gt_name   = classnames[gt_label]   if classnames else str(gt_label)
                                pred_name = classnames[pred_label] if classnames else str(pred_label)

                                viz_buffer.add(
                                    video_path    = video_path,
                                    final_weights = w_np,
                                    attn_weights_list = a_np,
                                    gt_label      = gt_name,
                                    pred_label    = pred_name,
                                    is_correct    = is_correct,
                                )

                                # 凑够 max_videos 就出一张图，tag 用批次号区分
                                if viz_buffer.ready():
                                    viz_buffer.render(
                                        save_dir = args.visualize_savedir,
                                        tag      = f"batch{i:04d}",
                                    )
                            else:
                                print(f"\n[Visualizer Warning] 匹配到 {video_path}，"
                                    f"但 final_weights 不存在！\n") 
                    # ============================================================
                    record = {
    ...
    if args is not None and args.visualize_video is not None:
        if viz_buffer.records:
            viz_buffer.render(save_dir=args.visualize_savedir, tag="final")
