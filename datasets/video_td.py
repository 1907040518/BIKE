...
class Video_dataset(data.Dataset):
    ...
    def _get_train_frame_index(self, num_frames, seg):
        ...
        return get_gop_pos(v_frame_idx, self.modality)
        ...
        if self.modality in ['RGB', 'video']:
            ...
            for seg in range(self.num_segments):   # 获取num segment
                ...
                img = load(video_path, gop_index, gop_pos,
                        representation_idx, self.accumulate)
                
                if img is None:
                    print('Error: loading video %s failed.' % video_path)
                    img = np.zeros((256, 256, 2)) if self.modality == 'mv' else np.zeros((256, 256, 3))
                else:
                    if self.modality == 'mv':
            ...
            if self.modality == 'iframe':
                ...
                input = (input - 0.5) / self.input_std
...
if __name__ == '__main__':
    ...
    '/datasets/hmdb51/mpeg4_videos', 'lists/hmdb51/train_rgb_split_1.txt',
    'lists/hmdb51/hmdb51_labels.csv', num_segments=12,
