...
class Video_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for seg in range(self.num_segments):
            # ==========================================
            # 核心修复 1：设置单一时间锚点 (Time Anchor)
            # 以 'mv' 为基准采样一次，保证取到的帧有运动信息 (gop_pos > 0)
            # ==========================================
            if not self.test_mode:
                gop_index_anchor, gop_pos_anchor = self._get_train_frame_index(num_frames, seg, "mv")
            else:
                gop_index_anchor, gop_pos_anchor = self._get_test_frame_index(num_frames, seg, "mv")

            # ==========================================
            # 核心修复 2：强行对齐物理时间结构
            # ==========================================
            gop_index_mv = gop_index_anchor
            gop_pos_mv = gop_pos_anchor
            
            gop_index_res = gop_index_anchor
            gop_pos_res = gop_pos_anchor
        ...
        # ==========================================
        # 核心修复 3：引入独立数据增强逻辑 (来自 video3.py)
        # ==========================================
        if isinstance(self.transform, dict):
            frames_iframe = self.transform['iframe'](frames_iframe)
            frames_mv = self.transform['mv'](frames_mv)
            frames_residual = self.transform['residual'](frames_residual)
        else:
            # 兼容普通 Compose 
