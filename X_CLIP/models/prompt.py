...
class MulitHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        B, N, C = q.shape
        B, M, C = k.shape
        # print("B, N, C = q.shape:",B,N,C)
        # print("B, M, C = q.shape:",B,M,C)
...
class VideoSpecificPrompt(nn.Module):
    def __init__(self, layers=2, embed_dim=512, alpha=0.1):
        super().__init__()
        # print("VideoPrompt Forard embed_dim:::::::")
        # print(embed_dim)
        # print(embed_dim//64)
    ...
    def forward(self, text, visual):    #
        ...
        B, N, C = visual.shape
        # print("B, N, C = visual.shape", B, N, C)
...
class Video_Prompt(nn.Module):
    def __init__(self, clip_state_dict):
            super().__init__()
            self.embed_dim = clip_state_dict["text_projection"].shape[1]
            self.video_prompt = VideoSpecificPrompt(embed_dim = self.embed_dim)

    def forward(self, text, visual):  

        return self.video_prompt(text, visual)

if __name__=='__main__':
    match=VideoSpecificPrompt(embed_dim = 768)
