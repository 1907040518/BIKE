...
if __name__=='__main__':
    ...
    feature_dim = getattr(model, 'head').in_features
    setattr(model, 'head', nn.Linear(feature_dim, 51))
    input=torch.rand(2,3,224,224)  # 2 8 3 224 224
    # input = rearrange(input, 'b t c h w -> b c t h w')
    input = input.to('cuda')
    model = model.to('cuda')  # 如果模型也在 CPU 上，确保将模型转移到 GPU 上
    out=model(input)
