...
def main(args):
    ...
    val_data = Video_compress_dataset(
        ...
        test_clips=args.test_clips, accumulate=(not args.no_accumulation), GOP_SIZE = config.data.GOP_SIZE)
    ...
    if args.distributed:
        ...
        if config.network.sim_header != "None":
            ...
            mv_head = DistributedDataParallel(mv_head.cuda(), device_ids=[args.gpu])
    classes,n_class = text_prompt(val_data, config)
...
def validate(val_loader, classes, device, model, video_head, mv_head, config, n_class, test_crops, test_clips):
    ...
    with torch.no_grad():
        ...
        for i, (image, mv, residual, class_id) in enumerate(val_loader):
            ...
            mv = mv.view((-1, n_seg, 2)+ mv.size()[-2:])  # Adjust if necessary
            residual = residual.view((-1, n_seg, 3) + residual.size()[-2:]) # Adjust if necessary
