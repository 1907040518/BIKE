...
from experiment_logging_template import ExperimentLogger, count_parameters
...
def main(args):
    ...
    logger.info(f"Trainable: {param_info['trainable_params']:,} ({param_info['trainable_ratio']:.1f}%)")
    
    exp_logger = ExperimentLogger("video_full_finetun")  # 改成您的实验名称
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        if (epoch+1) % config.logging.eval_freq == 0:
            ...
            exp_logger.log_epoch(epoch+1, train_loss, 0.0, prec1, current_lr)  # val_loss暂时设为0.0
            if dist.get_rank() == 0:
    ...
    # 在 for epoch 循环结束后添加
    summary = exp_logger.end_training()

    import json
    results = {**summary, **param_info}
    with open(f'{working_dir}/experiment_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    logger.info(f"=== Experiment Summary ===")
    logger.info(f"Best accuracy: {summary['best_val_acc']:.2f}%")
    logger.info(f"Trainable params: {param_info['trainable_ratio']:.1f}%")
    logger.info(f"Training time: {summary['total_training_time_hours']:.2f} hours")
