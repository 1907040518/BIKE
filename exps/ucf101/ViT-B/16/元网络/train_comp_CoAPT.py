...
def main(args):
    ...
    if coapt_bias_cfg is None:
        attr_cfg = config.network.get('attribute_prompt', None)
        if isinstance(attr_cfg, dict):
            coapt_bias_cfg = attr_cfg.get('coapt_bias', attr_cfg.get('coapt', None))
...
def validate(epoch, val_loader, classes, device, model, video_head, config, n_class, logger,
             return_sim=False, coapt_bias_enabled=False):
    top1 = AverageMeter()
    top5 = AverageMeter()
    sims_list = []
    labels_list = []
