...
class CoAPTBiasAdapter(nn.Module):
    """Meta网络，用于根据视频特征自适应调整文本特征（CoAPT核心思想）。"""
    ...
    def forward(self, video_features, text_features):
        ...
        Args:
            video_features: (B, C)
            text_features: (C,) or (N_cls, C) or (B, N_cls, C)
        Returns:
            调整后的文本特征，shape为(B, N_cls, C)
        """
        ...
        if text_features.dim() == 2:
            ...
            if text_features.size(0) != video_features.size(0):
                raise ValueError("当文本特征为三维张量时，其batch维度需与视频特征保持一致")
        else:
            raise ValueError("不支持的文本特征张量维度")

        video_features = F.normalize(video_features, dim=-1, eps=1e-6)
        text_features = F.normalize(text_features, dim=-1, eps=1e-6)

        video_expanded = video_features.unsqueeze(1).expand_as(text_features)
        fusion = torch.cat([text_features, video_expanded], dim=-1)
...
def main(args):
    ...
    if attribute_prompt_enabled:
        ...
        coapt_bias_cfg_raw = attribute_prompt_cfg.get('coapt_bias', attribute_prompt_cfg.get('coapt', None))
        if isinstance(coapt_bias_cfg_raw, dict):
            coapt_bias_enabled = coapt_bias_cfg_raw.get('enable', True)
        else:
            coapt_bias_enabled = True if coapt_bias_cfg_raw is None else bool(coapt_bias_cfg_raw)
    else:
    ...
    if coapt_bias_enabled:
        if isinstance(coapt_bias_cfg_raw, dict):
            if 'layers' in coapt_bias_cfg_raw and coapt_bias_cfg_raw['layers'] is not None:
                layers_cfg = coapt_bias_cfg_raw['layers']
                ...
                layer_vals = [coapt_bias_cfg_raw.get(key) for key in layer_keys if key in coapt_bias_cfg_raw]
                if layer_vals:
                    coapt_hidden_layers = [int(v) for v in layer_vals if v is not None]
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        train(model, video_head, train_loader, optimizer, criterion, scaler,
              epoch, device, lr_scheduler, config, classes, logger, attribute_prompt_enabled, coapt_bias_enabled)

        if (epoch+1) % config.logging.eval_freq == 0:
            if config.data.dataset == 'charades':
                prec1, output_list, labels_list = validate_mAP(
                    epoch, val_loader, classes, device, model, video_head,
                    config, n_class, logger, attribute_prompt_enabled=attribute_prompt_enabled, coapt_bias_enabled=coapt_bias_enabled)
            else:
                prec1, output_list, labels_list = validate(
                    epoch, val_loader, classes, device, model, video_head,
                    config, n_class, logger, return_sim=save_score,
                    attribute_prompt_enabled=attribute_prompt_enabled, coapt_bias_enabled=coapt_bias_enabled)
...
def train(model, video_head, train_loader, optimizer, criterion, scaler,
          ...
          attribute_prompt_enabled=False, coapt_bias_enabled=False):
    """ train a epoch """
        ...
        with autocast():
            if config.solver.loss_type in ['NCE', 'DS']:
                ...
                if coapt_bias_enabled and getattr(clip_model, "coapt_bias", None) is not None:
                    ...
                    text_embedding_for_allgather = None if text_embedding is None else text_embedding
                else:
                    text_embedding_for_allgather = text_embedding

                # gather操作
                image_embedding = allgather(image_embedding)
                if text_embedding_for_allgather is not None and text_embedding_for_allgather.dim() >= 3:
                    text_embedding = allgather(text_embedding_for_allgather)
                else:
                    text_embedding = text_embedding_for_allgather
...
def validate(epoch, val_loader, classes, device, model, video_head, config, n_class, logger,
             return_sim=False, attribute_prompt_enabled=False, coapt_bias_enabled=False):
    top1 = AverageMeter()
    top5 = AverageMeter()
    sims_list = []
    labels_list = []
    ...
    with torch.no_grad():
        ...
        for i,(image, mv, residual, class_id) in enumerate(val_loader):
            ...
            if coapt_bias_enabled and getattr(clip_model, "coapt_bias", None) is not None:
                ...
                cls_feat_for_batch = clip_model.coapt_bias(video_token, cls_feature)
            else:
                cls_feat_for_batch = cls_feature

            similarity = video_head(merged_feats, text_features, cls_feat_for_batch)
    ...
    with torch.no_grad():
        ...
        for i, (image, class_id) in enumerate(val_loader):
            ...
            if coapt_bias_enabled and getattr(clip_model, "coapt_bias", None) is not None:
                ...
                cls_feature_batch = clip_model.coapt_bias(video_token, cls_feature)
            else:
                cls_feature_batch = cls_feature

            similarity = video_head(image_features, text_features, cls_feature_batch)
