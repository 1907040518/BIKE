...
def unwrap_module(module):
    """Return the underlying module when using DistributedDataParallel."""
    return module.module if hasattr(module, "module") else module


class CoAPTPromptAdapter(nn.Module):
    """Meta-network that injects image-conditioned bias into text features."""

    def __init__(self, embed_dim, hidden_dims=None, normalize_inputs=True, detach_visual=False):
        super().__init__()
        if not hidden_dims:
        ...
        for hid in hidden_dims:
            hid = int(hid)
            layers.append(nn.Linear(input_dim, hid))
            layers.append(nn.ReLU(inplace=True))
            input_dim = hid
        layers.append(nn.Linear(input_dim, embed_dim))

        self.bias_meta_net = nn.Sequential(*layers)
    ...
    def forward(self, text_features, video_features):
        ...
        if txt.dim() == 2 and txt.size(0) == vid.size(0):
            txt_in = txt
            vid_in = vid.to(txt_in.dtype)
            if self.normalize_inputs:
                txt_in = F.normalize(txt_in, dim=-1)
                vid_in = F.normalize(vid_in, dim=-1)
            fusion = torch.cat([txt_in, vid_in], dim=-1)
            bias = self.bias_meta_net(fusion)
            return txt + bias

        if txt.dim() == 2:
            txt = txt.unsqueeze(0).expand(vid.size(0), -1, -1)
        elif txt.dim() == 3 and txt.size(0) != vid.size(0):
            txt = txt.expand(vid.size(0), -1, -1)

        vid = vid.unsqueeze(1).expand(-1, txt.size(1), -1).to(txt.dtype)
        if self.normalize_inputs:
            txt_in = F.normalize(txt, dim=-1)
            vid_in = F.normalize(vid, dim=-1)
        else:
            txt_in = txt
            vid_in = vid
...
def compute_dp_similarity(video_features, cls_embeddings):
    if video_features.dim() == 1:
        video_features = video_features.unsqueeze(0)
    ...
    if cls_embeddings.dim() == 2:
        ...
        return torch.matmul(video_features, cls_norm.t())

    if cls_embeddings.dim() == 3:
        cls_norm = F.normalize(cls_embeddings, dim=-1)
        return torch.einsum('bd,bkd->bk', video_features, cls_norm)

    raise ValueError(f"Unsupported cls_embeddings shape: {cls_embeddings.shape}")
...
def main(args):
    ...
    attribute_prompt_cfg = config.network.get('attribute_prompt', DotMap())
    attribute_prompt_enabled = bool(attribute_prompt_cfg.get('enable', False))
    attribute_prompt_learner = None

    if attribute_prompt_enabled:
        attribute_words = attribute_prompt_cfg.get('attributes', [])
        if isinstance(attribute_words, str):
            attribute_words = [attribute_words]
        if not attribute_words:
            raise ValueError("Attribute prompt enabled but no attributes were provided in config.network.attribute_prompt.attributes")
        ...
        class_specific = bool(attribute_prompt_cfg.get('class_specific', False))
        ...
        if dist.get_rank() == 0:
            logger.info(f"Attribute prompt branch enabled with attributes: {attribute_words}")
            logger.info(f"Attribute prompt tokens -> context: {num_ctx_tokens}, attribute_ctx: {attr_ctx_tokens_list}, class_specific: {class_specific}")
    else:
        classes, n_class = text_prompt(train_data)    # torch.Size([51, 77])    使用vita的时候，返回的是类别名
        classes = classes.to(device)


    coapt_cfg = config.network.get('coapt', DotMap())
    coapt_enabled = bool(coapt_cfg.get('enable', False))
    if coapt_enabled and config.network.interaction != 'DP':
        if dist.get_rank() == 0:
            logger.warning("CoAPT 元提示当前仅支持 DP 交互方式，将回退到基础策略。")
        coapt_enabled = False

    if coapt_enabled:
        embed_dim = model.text_projection.shape[1]
        hidden_dims = coapt_cfg.get('hidden_dims', None)
        if hidden_dims is None:
            candidate_dims = [coapt_cfg.get('layer1'), coapt_cfg.get('layer2'), coapt_cfg.get('layer3')]
            hidden_dims = [int(v) for v in candidate_dims if v is not None]
        if not hidden_dims:
            hidden_dims = [embed_dim * 2, embed_dim]

        normalize_inputs = bool(coapt_cfg.get('normalize_inputs', True))
        detach_visual = bool(coapt_cfg.get('detach_visual', False))

        coapt_adapter = CoAPTPromptAdapter(
            embed_dim=embed_dim,
            hidden_dims=hidden_dims,
            normalize_inputs=normalize_inputs,
            detach_visual=detach_visual,
        )
        coapt_adapter.to(dtype=model.text_projection.dtype)
        model.coapt_adapter = coapt_adapter
        model.coapt_active = True
        model.coapt_cfg = coapt_cfg

        if dist.get_rank() == 0:
            logger.info(
                f"CoAPT 元提示已启用，隐藏层: {hidden_dims}, normalize_inputs: {normalize_inputs}, detach_visual: {detach_visual}"
            )
    else:
        model.coapt_adapter = None
        model.coapt_active = False
        model.coapt_cfg = coapt_cfg
        ...
        for name, param in model.named_parameters():
            if "visual" not in name and "logit_scale" not in name and "beta" not in name:
                if attribute_prompt_enabled and "attribute_prompt_learner" in name:
                    continue
                if getattr(model, "coapt_active", False) and "coapt_adapter" in name:
                    continue
    ...
    if config.solver.evaluate:
        ...
        if config.data.dataset == 'charades':
            prec1, output_list, labels_list = validate_mAP(
                ...
                model, video_head, config, n_class, logger, attribute_prompt_enabled=attribute_prompt_enabled)
        else:
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        train(model, video_head, train_loader, optimizer, criterion, scaler,
              epoch, device, lr_scheduler, config, classes, logger, attribute_prompt_enabled)

        if (epoch+1) % config.logging.eval_freq == 0:
            if config.data.dataset == 'charades':
                prec1, output_list, labels_list = validate_mAP(
                    epoch, val_loader, classes, device, model, video_head,
                    config, n_class, logger, attribute_prompt_enabled=attribute_prompt_enabled)
            else:
                prec1, output_list, labels_list = validate(
                    epoch, val_loader, classes, device, model, video_head,
...
def train(model, video_head, train_loader, optimizer, criterion, scaler,
          epoch, device, lr_scheduler, config, classes, logger, attribute_prompt_enabled=False):
    """ train a epoch """
    ...
    head_module = unwrap_module(video_head)
    coapt_adapter = getattr(base_model, "coapt_adapter", None)
    coapt_active = getattr(base_model, "coapt_active", False) and coapt_adapter is not None and config.network.interaction == 'DP'
    ...
    for i,(images, mvs, residuals,list_id) in enumerate(train_loader):  
        ...
        with autocast():
            if config.solver.loss_type in ['NCE', 'DS']:
                ...
                if coapt_active:
                    video_tokens_local = head_module.agg_video_feat(image_embedding)
                    video_features_local = video_tokens_local.mean(dim=1)
                    cls_embedding = coapt_adapter(cls_embedding, video_features_local)
...
def validate(epoch, val_loader, classes, device, model, video_head, config, n_class, logger, return_sim=False, attribute_prompt_enabled=False):
    top1 = AverageMeter()
    top5 = AverageMeter()
    sims_list = []
    labels_list = []
    ...
    model.eval()
    video_head.eval()
    clip_model = unwrap_module(model)
    head_module = unwrap_module(video_head)
    coapt_adapter = getattr(clip_model, "coapt_adapter", None)
    coapt_active = getattr(clip_model, "coapt_active", False) and coapt_adapter is not None and config.network.interaction == 'DP'

    with torch.no_grad():
        if attribute_prompt_enabled:
            cls_feature, text_features = clip_model.encode_attribute_prompts(return_token=True)
        else:
            text_inputs = classes.to(device)
            cls_feature, text_features = clip_model.encode_text(text_inputs, return_token=True)
        print("")
        for i,(image, mv, residual, class_id) in enumerate(val_loader):
            ...
            if coapt_active:
                video_tokens = head_module.agg_video_feat(merged_feats)
                video_features = video_tokens.mean(dim=1)
                adapted_cls = coapt_adapter(cls_feature, video_features)
                similarity = compute_dp_similarity(video_features, adapted_cls)
            else:
                similarity = head_module(merged_feats, text_features, cls_feature)
...
def validate_mAP(epoch, val_loader, classes, device, model, video_head, config, n_class, logger, attribute_prompt_enabled=False):
    mAP = AverageMeter()
    model.eval()
    video_head.eval()
    clip_model = unwrap_module(model)
    ...
    with torch.no_grad():
        ...
        for i, (image, class_id) in enumerate(val_loader):
            ...
            if coapt_active:
                video_tokens = head_module.agg_video_feat(image_features)
                video_features = video_tokens.mean(dim=1)
                adapted_cls = coapt_adapter(cls_feature, video_features)
                similarity = compute_dp_similarity(video_features, adapted_cls)
            else:
                similarity = head_module(image_features, text_features, cls_feature)
